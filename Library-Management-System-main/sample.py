...
def open_admin_panel(root):
    ...
    def open_admin_management():
        ...
        def open_input_panel(action):
            ...
            def perform_action():
                ...
                if action == "Insert":
                    ...
                    mydb.commit()
                    result_label.config(text=f"🗑 Deleted Admin '{admin_id}'")
        ...
        actions = ["Insert", "Delete", "Search", "Update", "View All"]
        for act in actions:
            if act == "View All":
    ...
    tk.Button(panel, text="User Management", font=("Segoe UI", 12), bg="#D2B48C",
              command=lambda: open_user_panel(panel)).pack(pady=10, fill="x", padx=30)

    tk.Button(panel, text="Book Management", font=("Segoe UI", 12), bg="#D2B48C",
...
def open_user_panel(root):
    ...
    def display_users():
        ...
        formatted = []
        for user in records:
            book_list = user[7]  # BorrowedBooks
            borrowed = book_list.split(",") if book_list else []
            book_names = []
            for b in borrowed:
            ...
            formatted.append((user[0], user[1], user[2], ", ".join(book_names) if book_names else "None",
                              user[3] or "N/A", user[4], user[5] or "N/A"))

        show_popup("User Records", formatted, root,
                   headers=["UserID", "UserName", "Password", "BooksIssued", "DateBorrowed",
                            "NoOfBooksBorrowed", "DueDate"])
...
def open_book_panel(root):
    def show_book_stats():
        ...
        stats_popup.configure(bg="white")
    ...
    def view_books():
        ...
        popup.configure(bg="#FFC0CB")  # pink background

        tk.Label(popup, text="📚 View All Books", font=("Segoe UI", 14, "bold"), bg="#FFC0CB").pack(pady=10)

        # --- Search bar
        search_frame = tk.Frame(popup, bg="#FFC0CB")
        search_frame.pack(pady=5)
        tk.Label(search_frame, text="Search:", font=("Segoe UI", 11), bg="#FFC0CB").pack(side="left", padx=5)
        search_entry = tk.Entry(search_frame, width=50, font=("Segoe UI", 11))
        search_entry.pack(side="left", padx=5)

        # --- Scrollable table area
        table_frame = tk.Frame(popup, bg="#FFC0CB")
        ...
        def display_books_in_popup(keyword=""):
            ...
            for user_id, borrowed_books in mycursor.fetchall():
                if borrowed_books:
                    for b in borrowed_books.split(","):
                        borrowed_map[str(b).strip()] = user_id   # normalize to string

            # Display rows
            for row_index, row in enumerate(book_records, start=1):
                book_id = str(row[0]).strip()
                is_borrowed = book_id in borrowed_map
                row_bg = "#ffcccc" if is_borrowed else "white"   # 🔴 highlight row if borrowed
                ...
                status_bg = "#ffeaa7" if is_borrowed else "#b8e994"
...
def open_logged_in_user_panel(user_id):
    ...
    def view_available_books():
        ...
        def display_available_books(keyword=""):
            ...
            for row in mycursor.fetchall():
                borrowed_ids.update([b.strip() for b in row[0].split(",") if b.strip()])

            # only available books
            available_books = [book for book in all_books if book[0] not in borrowed_ids]
    ...
    def borrow_book(user_id):
        ...
        tk.Label(popup, text="Enter Book ID to Borrow:", font=("Segoe UI", 12), bg="white").pack(pady=10)
        book_entry = tk.Entry(popup, font=("Segoe UI", 12))
        book_entry.pack(pady=10)

        def confirm_borrow():
            book_id = book_entry.get().strip()
            if not book_id:
                messagebox.showerror("Error", "Enter Book ID", parent=popup)
                return
            ...
            book = mycursor.fetchone()
            if not book:
                messagebox.showerror("Error", f"Book ID {book_id} not found.", parent=popup)
                return

            # --- Get current borrowed books for user ---
            mycursor.execute("SELECT BorrowedBooks FROM UserRecord WHERE UserID = %s", (user_id,))
            record = mycursor.fetchone()
            borrowed_books = record[0] if record and record[0] else ""

            borrowed_list = [b.strip() for b in borrowed_books.split(",") if b.strip()]
            if book_id in borrowed_list:
                messagebox.showwarning("Warning", f"You already borrowed Book ID {book_id}.", parent=popup)
                return

            # --- Add new book to borrowed list ---
            borrowed_list.append(book_id)
            new_borrowed_books = ",".join(borrowed_list)

            # --- Set dates ---
            today = date.today()
            due_date = today + timedelta(days=14)  # 📅 2 weeks loan

            # --- Update UserRecord ---
            mycursor.execute("""
                UPDATE UserRecord
                SET BorrowedBooks = %s, DateBorrowed = %s, DueDate = %s
                WHERE UserID = %s
            """, (new_borrowed_books, today, due_date, user_id))
            mydb.commit()

            messagebox.showinfo("Success", f"Book {book_id} borrowed successfully!\nDue Date: {due_date}", parent=popup)
            popup.destroy()

        tk.Button(popup, text="Borrow", font=("Segoe UI", 12), bg="#a8e6cf", command=confirm_borrow).pack(pady=20)
    ...
    def return_book(user_id):
        ...
        tk.Label(popup, text="Enter Book ID to Return:", font=("Segoe UI", 12), bg="white").pack(pady=10)
        book_entry = tk.Entry(popup, font=("Segoe UI", 12))
        book_entry.pack(pady=10)
        ...
        def confirm_return():
            ...
            if book_id not in borrowed_list:
                messagebox.showerror("Error", f"Book ID {book_id} is not borrowed.", parent=popup)
                return
            ...
            # --- Reset DueDate if no books left ---
            new_due_date = None if not borrowed_list else date.today() + timedelta(days=14)

            # --- Update UserRecord ---
            mycursor.execute("""
                UPDATE UserRecord
                SET BorrowedBooks = %s, DueDate = %s
                WHERE UserID = %s
            """, (new_borrowed_books, new_due_date, user_id))
            mydb.commit()

            messagebox.showinfo("Success", f"Book {book_id} returned successfully!", parent=popup)
            popup.destroy()

        tk.Button(popup, text="Return", font=("Segoe UI", 12), bg="#ffb3b3", command=confirm_return).pack(pady=20)
    ...
    # -------- Buttons --------
    tk.Button(panel, text="View Books", font=("Segoe UI", 12), bg="#a8e6cf",
          command=view_available_books).pack(pady=10, fill="x", padx=30)

    tk.Button(panel, text="Borrow Book", font=("Segoe UI", 12), bg="#ffe0ac",
            command=lambda: borrow_book(user_id)).pack(pady=10, fill="x", padx=30)

    tk.Button(panel, text="Return Book", font=("Segoe UI", 12), bg="#ff8b94",
            command=lambda: return_book(user_id)).pack(pady=10, fill="x", padx=30)

    tk.Button(panel, text="My Book Details", font=("Segoe UI", 12), bg="#d1c4e9",
        command=my_book_details).pack(pady=10, fill="x", padx=30)
...
def login():
    ...
    if role == "admin":
        ...
        if result and result[0] == pwd:
            messagebox.showinfo("Success", f"Welcome Admin {uid}!")
            open_admin_panel(root)
        else:
            messagebox.showerror("Login Failed", "Incorrect credentials.")
        return

    # For user login, fetch UserType as well
    mycursor.execute("SELECT Password, UserType FROM UserRecord WHERE UserID = %s", (uid,))
    result = mycursor.fetchone()

    if result:
        print("Fetched result:", result)  # Debug print
        password_from_db, user_type = result
        print("UserType from DB:", user_type)  # Debug print
        if password_from_db == pwd:
            if user_type is None:
                messagebox.showerror("Error", "User type not set in database.")
                return
            print("Role selected:", role)  # Debug print
            print("UserType:", user_type)  # Debug print
            if user_type.lower() != role:
                messagebox.showerror("Error", f"This user is a {user_type}, please select the correct role.")
                return
